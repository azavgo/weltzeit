...
use chrono_tz::Tz;
use chrono_tz::Europe::{Moscow, Budapest, Vienna, Berlin, London};
use chrono_tz::Australia::{Sydney, Brisbane, Melbourne, Hobart, Adelaide, Darwin, Perth};
use chrono_tz::US::{Pacific, Central, Eastern};
use chrono_tz::Pacific::Auckland;

#[derive(Debug)]
pub enum CityList {
    Moscow, Budapest, Vienna, Berlin, London,
    Sydney, Brisbane, Melbourne, Hobart, Adelaide, Darwin, Perth,
...
impl City {
    ...
    pub fn current_time(&self) -> String {
        format!("{:?}: {}/{}/{} {}:{}:{}", self.city(), self.day(), self.month(), self.year(), self.hours(), self.minutes(), self.seconds())
    }

    pub fn timestamp(&self) -> i64 {
        let dt: DateTime<Utc> = 
            Utc.with_ymd_and_hms(self.year(), 
                                self.month(), 
                                self.day(), 
                           self.hours(), 
                            self.minutes(),
